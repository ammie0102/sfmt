...
app.post("/food/add", upload.single("foodMenuImage"), async (req, res) => {
  try {
    const { foodMenuID, foodMenuName, foodMenuDescription, foodMenuCategory } =
      req.body;
...
app.post("/food/price/add", async (req, res) => {
  ...
  for (let i = 0; i < 20; i++) {
    foodPriceID += possible.charAt(Math.floor(Math.random() * possible.length));
  }
...
app.get("/food/price", async (req, res) => {
  try {
    const allFoodPrice = await pool.query("SELECT * FROM foodMenuPriceTable");
...
app.patch("/food/update/:id", async (req, res) => {
  try {
    ...
    const updateFood = await pool.query(
      ...
      [foodMenuName, foodMenuDescription, foodMenuCategory, foodMenuImage, id]
    );
