...
const Menu = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const responseRider = await fetch("http://localhost:7722/rider");
        ...
        if (localStorage.getItem("userID") !== null) {
          const responseCart = await fetch(
            "http://localhost:7722/cart/" + localStorage.getItem("userID")
          );
  ...
  const findNearestBranch = (customerLatitude, customerLongitude) => {
    ...
    branchLocation.forEach((branch) => {
      ...
      const dLon = ((branchLongitude - customerLongitude) * Math.PI) / 180;
      const a =
    ...
    const order = {
      ...
      customerordertotalprice: cart.reduce(
        (total, item) =>
          ...
          parseFloat(
            foodPrices.find(
              (price) => price.foodmenupriceid === item.foodmenupriceid
            ).foodmenuprice
          ),
        0
      ),
      ...
      deliverypersonid: rider[0].deliverypersonid,
    };
    console.log(order);
    try {
      ...
      if (response.ok) {
        cart.forEach(async (item) => {
          const orderItem = {
            customerorderitemid: Math.random().toString(36).substr(2, 9),
        ...
        try {
          const responseDeleteCart = await fetch(
            "http://localhost:7722/customer/cart/delete/" + customerID,
            {
              method: "DELETE",
            }
          );
      ...
      <section className="menu-section relative">
        <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 h-full">
          <div className="flex flex-row h-full">
                          ...
                          <div className="food-menu-info-container p-4 ">
                            ...
                            <div className="food-menu-price-container">
                              {foodPrices
                                .filter(
                                  (price) =>
                                    price.foodmenuid === food.foodmenuid
                                )
                                .map((price) => (
                                  <div key={price.foodmenupriceid}>
                                    <p className="text-sm text-gray-500">
                                      Php {price.foodmenuprice} /{" "}
            ...
            <div className="order-container border border border-red-500 h-full w-[36%] p-4">
              ...
              <div className="order-address-container">
                ...
                <div className="flex flex-row justify-between items-center space-x-2 mt-2">
                  {address.length > 0 ? (
                    ...
                    >
                      <FontAwesomeIcon icon={faEdit} /> Change Address
                    </button>
                  ) : null}
                  <Link
                    ...
                    to="/address"
                  >
                    <FontAwesomeIcon icon={faSquarePlus} /> Add New Address
                  </Link>
                </div>
              </div>
              <hr className="my-2 border-b-1 border-gray-300" />
                  ...
                  >
                    {branchLocation.map((location) => (
                      <option
                        ...
                        selected={
                          location.branchid === nearestBranch ? true : false
                        }
                      >
                        {location.branchname}{" "}
                        {location.branchid === nearestBranch
              ...
              <div className="order-items-container mt-4">
                ...
                <div className="order-items-body">
                        ...
                        >
                          <div className="flex flex-row justify-start items-center space-x-2">
                            ...
                            <p className="text-sm font-medium text-gray-900">
                              {
                                foods.find(
                                  (food) => food.foodmenuid === item.foodmenuid
                                ).foodmenuname
                              }
                            </p>
                          </div>
                  ...
                  <div className="flex flex-col">
                    <button className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm mt-4 transition duration-150 ease-in-out">
                      View Cart
                    </button>
                    <button
