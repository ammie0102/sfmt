...
const FoodModal = ({ showModal, setShowModal, selectedFood, foodPrices }) => {
  ...
  const [foodMenuID, setFoodMenuID] = useState(selectedFood.foodmenuid);
    ...
    >
      <div className="relative w-full max-w-2xl max-h-full">
        <div className="relative bg-white rounded-lg shadow">
          <div className="flex items-start justify-between p-4 border-b rounded-t relative">
          ...
          <div className="p-6 space-y-2 overflow-y-auto h-96">
            ...
            <div className="input-group">
              <select
                ...
                name="foodSize"
                required
                onChange={(e) => setFoodMenuPriceID(e.target.value)}
              >
                {/* {
            foodmenupriceid: "FP8",
            foodMenuID: "FM4",
            foodmenuprice: 12.99,
            foodMenuCutType: "Half",
        }, */}
                {foodPrices
                  .filter(
                    (foodPrice) =>
                      foodPrice.foodMenuID === selectedFood.foodMenuID
                  )
                  .map((filteredFoodPrice) => (
                    <option
                      key={filteredFoodPrice.foodmenupriceid}
                      value={filteredFoodPrice.foodmenupriceid}
                    >
                      Php {filteredFoodPrice.foodmenuprice} /{" "}
                      {filteredFoodPrice.foodMenuCutType}
                    </option>
                  ))}
              </select>
            </div>
